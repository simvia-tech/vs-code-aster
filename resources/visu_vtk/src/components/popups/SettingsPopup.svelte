<script lang="ts">
  ...
  let hiddenOpacityPct = $derived(
    Math.round($settings.hiddenObjectOpacity * 100),
  );
  let edgeThresholdDisplay = $derived(
    parseFloat($settings.edgeThresholdMultiplier.toFixed(2)),
  );
  let edgeModeLabel = $derived(
    edgeModeOptions.find((o) => o.value === $settings.edgeMode)?.label ??
      $settings.edgeMode,
  );
  ...
  let groupTransparencyPct = $derived(
    Math.round($settings.groupTransparency * 100),
  );
  ...
  function resetEdgesTab() {
    ...
    GlobalSettings.Instance.edgeThresholdMultiplier =
      EDGE_DEFAULTS.edgeThresholdMultiplier;
  ...
  function resetVisibilityTab() {
    GlobalSettings.Instance.hiddenObjectOpacity =
      VISIBILITY_DEFAULTS.hiddenObjectOpacity;
    GlobalSettings.Instance.groupTransparency =
      VISIBILITY_DEFAULTS.groupTransparency;
  ...
  function resetDisplayTab() {
    GlobalSettings.Instance.showOrientationWidget =
      DISPLAY_DEFAULTS.showOrientationWidget;
...
{#snippet tip(text: string)}
  <span
    class="relative group cursor-help shrink-0"
...
<div
  ...
  role="document"
>
  <div class="absolute inset-0 rounded backdrop-blur-lg pointer-events-none" style="background: var(--ui-popup-bg)"></div>
  <nav
    class="relative z-10 flex flex-col py-4 shrink-0 w-36"
    style="border-right: 1px solid var(--ui-border)"
  >
    <span
      class="font-bold text-sm px-4 pb-3"
      style="color: var(--ui-text-primary)">Settings</span
    >
    {#each tabs as tab}
      <button
          ...
          {#if showThresholdSection}
            <div class="flex flex-col space-y-1.5 pt-1">
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs" style="color: var(--ui-text-secondary)">
                  {edgeThresholdDisplay.toPrecision(
                    edgeThresholdDisplay < 1 ? 2 : 3,
                  )}×
                </span>
              </div>
              <input
      ...
      {:else if activeTab === "Visibility"}
        <div class="flex flex-col space-y-2">
          <div class="flex flex-col space-y-1.5">
            ...
            />
            <span class="text-xs" style="color: var(--ui-text-secondary)"
              >When hiding an object with the eye button, it can remain slightly
              visible as a ghost.</span
            >
          </div>
          <div class="flex flex-col space-y-1.5 pt-1">
            <div class="flex items-center justify-between">
